...
public final class StudentAnalytics {
    ...
    public double averageAgeOfEnrolledStudentsParallelStream(
            final Student[] studentArray) {
        if (studentArray == null) throw new UnsupportedOperationException();
        double res =  Stream.of(studentArray)
                            .parallel()
                            .filter(Student::checkIsCurrent) //.filter(s -> s.checkIsCurrent())
                            .mapToDouble(Student::getAge) //.mapToDouble(s -> s.getAge())
                            .average()
                            .getAsDouble();
        return res;
    }
    ...
    public int countNumberOfFailedStudentsOlderThan20ParallelStream(
            final Student[] studentArray) {
        if (studentArray == null) throw new UnsupportedOperationException();
        int res = (int)Stream.of(studentArray)
                                .parallel()
                                .filter(s -> !s.checkIsCurrent())
